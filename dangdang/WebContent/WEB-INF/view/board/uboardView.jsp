...
<%
	...
	int endRow = currentPage * pageSize;
 	...
 	System.out.println(userId.getKgName());
 	
	count = diaryService.selectCountPage(userId.getKgName()); // 데이터베이스에 저장된 총 갯수

 	List<Diary> list = null;
	if (count > 0) {
		// getList()메서드 호출 / 해당 레코드 반환
		list = diaryService.selectDiaryPage(startRow, endRow, userId.getKgName());		
					...
					<table class = "table" align="center">
						...
						<tr>
							<td align="center" colspan="4" style="font-size: 0.7vw">
								...
								%>
											<a href="/diary/userview.do?pageNum=<%=startPage - 10%>">[이전]</a>	
								<%			
										}
								...
								%>
												<a href="/diary/userview.do?pageNum=<%=i%>">[<%=i %>]</a>
								<%	
											}
										} // for end
								...
								%>
											<a href="/diary/userview.do?pageNum=<%=startPage + 10 %>">[다음]</a>
								<%			
										}
									}
								%>
